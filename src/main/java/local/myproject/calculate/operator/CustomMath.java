package local.myproject.calculate.operator;
...
public class CustomMath {
    private static final Logger logger = LoggerFactory.getLogger(CustomMath.class.getName());
    ...
    public static double factorial(double numb)
    {
       if (numb < 0) {
           throw new ArithmeticException(String.format("Factorial argument must be a positive, but it's %s",
                   local.myproject.calculate.converter.Converter.doubleToString(numb)));
       }
       if(local.myproject.calculate.converter.Converter.doubleToString(numb).contains(".")) {
