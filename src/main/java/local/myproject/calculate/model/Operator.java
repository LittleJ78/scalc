...
public class Operator extends Unit {
    ...
    private TypeOfOperators setType(Operators value) {
        String operatorPackage = TypeOfOperators.class.getPackageName() + ".";
        List<String> typeOp = Arrays.stream(TypeOfOperators.values()).map(x -> x.name()).toList();
        for (int i = 0; i < typeOp.size(); i++) {
            try {
                Field[] fieldsOfEnumDifferentOperators = Class.forName(operatorPackage + typeOp.get(i)).getFields();
                ...
                logger.error("Не удалось определить тип оператора для {}", this.value);
            }
        }
        return null;
    }
